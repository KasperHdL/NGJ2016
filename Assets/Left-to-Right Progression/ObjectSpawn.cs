...
public class ObjectSpawn : MonoBehaviour {
	...
	void Awake () {
        ...
        ArrayManager(Instantiate(setOfGrapPoints[6],
                                    new Vector3(_cameraBounds.center.x, _cameraBounds.center.y+ 0.79f,-18),
                                    setOfGrapPoints[6].transform.rotation),
                                    ActiveGrapPoints);
        ArrayManager(Instantiate(setOfGrapPoints[1],
                                    new Vector3(_cameraBounds.center.x + ((_cameraBounds.extents.x/1.75f) * sizeToVectorFactor), _cameraBounds.center.y+ 0.79f,-18),
                                    setOfGrapPoints[1].transform.rotation),
                                    ActiveGrapPoints);
        ArrayManager(Instantiate(setOfGrapPoints[2],
                                    new Vector3((_cameraBounds.center.x + (_cameraBounds.extents.x*1.075f* sizeToVectorFactor)), _cameraBounds.center.y+ 0.79f,-18),
	...
	void Update ()
    {
        ...
        if (_CameraPrePos.x < (_cameraBounds.center.x- (_cameraBounds.extents.x/1.75) * sizeToVectorFactor))
        {
            ...
            ArrayManager(Instantiate(setOfGrapPoints[index],
                                    new Vector3((_cameraBounds.center.x + (_cameraBounds.extents.x * sizeToVectorFactor)), _cameraBounds.center.y+ 0.79f,-18),
